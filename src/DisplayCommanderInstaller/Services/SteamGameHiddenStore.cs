...
/// <summary>Persisted hidden Steam AppIds — excluded from All / Favorites / RenoDX until unhidden.</summary>
public sealed class SteamGameHiddenStore
{
    ...
    public bool IsHidden(uint appId)
    {
        lock (_lock)
        {
            EnsureLoaded();
    ...
    public void SetHidden(uint appId, bool hidden)
    {
        lock (_lock)
        {
            EnsureLoaded();
